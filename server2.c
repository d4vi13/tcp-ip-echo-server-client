...
#define PORT 10000
#define FILE_NAME "resultado"

int main(){
	int sock,sock1,fd,tamanho_arquivo,dados_restantes,len;
	...
	*/
	...
	//converte numerto na string pra int
	tamanho_arquivo = atoi(buffer);

	fd = open(FILE_NAME,O_WRONLY);
	dados_restantes = tamanho_arquivo;

	while (dados_restantes > 0){
		printf("Faltam receber %d\n",dados_restantes);
		len = recv(sock1,buffer,BUFFER_SIZE,0);
		dados_restantes = dados_restantes - len;
		printf("recebidos %d\n",len);
		//printf("buffer %s\n", buffer);
